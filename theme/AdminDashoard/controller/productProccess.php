...
if(isset($_POST['viewProduct'])){
  ...
  if(isset($_POST['action']) && $_POST['action'] === 'displayVerifyProduct'){
    ...
    if($data){
        ...
        $message = "<p class='text-center text-info'>No verify Ads Found</p>";
        echo $message;
    }
}  
...
if(isset($_POST['viewVerifyProduct'])){
  ...
  if(isset($_POST['action']) && $_POST['action'] === 'displayDisapproveProduct'){
    ...
    if($data){
        foreach($data as $datas){
            if(($datas['sipmUser_AdsVerified']) && $datas['sipmUser_AdsImgVerified'] == 2){
                $productStatus = "Disapprove";
            }else{
                $productStatus = "Not verify";
            }
            $output .='<tr>
            ...
            <td>
              <button data-toggle="modal" class="btn btn-info p-0 px-1 mb-1 viewProduct" id="'.$datas['sipmuser_PostId'].'" data-target="#viewProductDetails" title="view ads"><i class="fa fa-eye"></i></button>
              <button class="btn btn-success p-0 px-1 verifyProduct" id="'.$datas['sipmuser_PostId'].'" title="verify ads"><i class="fa fa-check"></i></button>
            </td>
          </tr>';
        }
        echo $output;
    }else{
        $message = "<p class='text-center text-info'>No disapprove Ads Found</p>";
        echo $message;
    }
